...
setup_github_ssh_rewrite () {
  ...
  # silently skipped). Generate it only where the GitHub SSH key is set up —
  # github::set_ssh_key writes the "Host github.com" block this checks for.
  # Headless workers (gantry) never have that block and must stay on HTTPS:
  # a global https->ssh rewrite there collides with their ssh->https rule and
  # git-lfs resolves the duplicate alias nondeterministically, killing ~50%
  # of pushes in LFS repos.
  if grep -qs '^Host github.com$' "$HOME/.ssh/config"; then
    printf '%s\n' \
      '# Generated by modules/git/setup.sh: this host has an authorised GitHub' \
      '# SSH key ("Host github.com" in ~/.ssh/config), so HTTPS remotes rewrite' \
      '# to SSH. See the include comment in modules/git/gitconfig.' \
      '[url "git@github.com:"]' \
