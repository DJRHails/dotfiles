...
# HTTPS -> SSH rewrite belongs on hosts where github::set_ssh_key provisioned
# a key (it writes the "Host github.com" block), and must not exist on
# headless workers, which push over HTTPS with a token. Running this daily
# migrates hosts that were provisioned before the rewrite moved out of the
# tracked gitconfig, without waiting for a manual setup run.
ensure_github_ssh_rewrite() {
  local rewrite="$HOME/.gitconfig.github-ssh"
  if grep -qs '^Host github.com$' "$HOME/.ssh/config"; then
    if [ ! -f "$rewrite" ]; then
      printf '%s\n' \
        ...
        '[url "git@github.com:"]' \
        '	insteadOf = https://github.com/' > "$rewrite"
      log "generated $rewrite (host has a GitHub SSH key)"
    fi
  fi
}
