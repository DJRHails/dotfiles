...
- Chrome (or Chromium, Brave, Edge, Vivaldi, **Arc**) with remote debugging enabled: open `chrome://inspect/#remote-debugging` and toggle the switch
- Node.js 22+ (uses built-in WebSocket)
- If your browser's `DevToolsActivePort` is in a non-standard location, set `CDP_PORT_FILE` to its full path
- **Arc Browser**: auto-detected at `~/Library/Application Support/Arc/User Data/DevToolsActivePort`. Arc may need to be launched with `--remote-allow-origins=*` if WebSocket connections are rejected (403). The script auto-discovers the live browser WebSocket URL via `/json/version` to handle Arc's stale port file IDs.
