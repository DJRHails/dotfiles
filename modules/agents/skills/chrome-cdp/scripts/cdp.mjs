...
async function getWsUrl() {
  ...
  const port = lines[0];
  // Use localhost — Chrome/Arc rejects WebSocket upgrades with IP-based Host headers (403 Forbidden).
  const host = process.env.CDP_HOST || 'localhost';
  // The browser ID in DevToolsActivePort can go stale (especially Arc).
  // Query /json/version for the live webSocketDebuggerUrl when possible.
  try {
    ...
    if (resp.ok) {
      ...
      if (info.webSocketDebuggerUrl) {
        // Rewrite host to localhost in case the browser reports 127.0.0.1
        return info.webSocketDebuggerUrl.replace('ws://127.0.0.1:', `ws://${host}:`);
      }
    }
  } catch { /* fall through to port file path */ }
